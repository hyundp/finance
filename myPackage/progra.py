...
def dbgout(message):
    ...
    post_message(myToken,"#finance", strbuf)
...
for d in dk:
    ...
    for i in range(0, len(dj.close)):
        ...
        #     end_date = end_date.strftime('%Y-%m-%d')
        if dj.PB.values[i] < 0.05 and dj.IIP21.values[i] > 0 and dj.volume[i] > 70000:
            dbgout("`볼랜더매수" + ' : ' + x + "`")
        if dj.PB.values[i] > 0.95 and dj.IIP21.values[i] < 0 and dj.volume[i] > 70000:
            dbgout("`볼랜더매도" + ' : ' + x + "`")
